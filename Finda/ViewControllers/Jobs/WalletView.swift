...
open class WalletView: UIView {
    ...
    open func removeAll(completion: RemovalCompletion? = nil) {
        for cardView in self.insertedCardViews {

            let removalBlock = removalAnimation(duration: 0.0)
            present(cardView: cardView, animated: false, completion: { [weak self] (_) in
                self?.presentedCardView = nil
                removalBlock(cardView, completion)
            })
        }
    ...
    func removalAnimation(duration: TimeInterval = WalletView.removalAnimationSpeed) -> RemovalAnimation {
