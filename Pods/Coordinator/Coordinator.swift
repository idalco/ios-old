...
open class Coordinator<T: UIViewController>: UIResponder, Coordinating {
    ...
    fileprivate(set) public var coordinatorStack: [Coordinating] = []
	...
	*/
	public func startChild(coordinator: Coordinating, completion: @escaping () -> Void = {}) {
        ...
        print("coordinatorStack: \(coordinatorStack)")
	...
	*/
	public func stopChild(coordinator: Coordinating, completion: @escaping () -> Void = {}) {
		...
		coordinator.stop {
            ...
            self.coordinatorStack = self.coordinatorStack.filter() { $0 !== coordinator }
            self.coordinatorStack.last?.activate()
            print("coordinatorStack: \(self.coordinatorStack)")
			completion()
		}
	}
}
