...
//            }
            ...
            if let transitionCoordinator = navigationController.transitionCoordinator {
                if !transitionCoordinator.isInteractive {
                    // non-interactive, just get the FROM coordinator in the NC transition
                    let fromViewController = transitionCoordinator.viewController(forKey: .from)
                    self.willShowController(viewController, fromViewController: fromViewController)
                } else {
                    // catch interactive transitions and set handler so don't miss them
                    navigationController.transitionCoordinator?.notifyWhenInteractionChanges {
                        [unowned self] context in
...
//        }
        ...
        // check if incoming vc is ours
        if viewControllers.contains( viewController ) || viewControllers.contains(fromViewController) {
               // we are moving to one of our view controllers
    
                    //    check is this pop:
                    if let vc = self.viewControllers.last, vc === fromViewController {
                            //    we are popping from our most recent vc, remove this controller from Coordinator's list
                                self.viewControllers.removeLast()
                            self.handlePopBack(to: self.viewControllers.last)
                        }
                //    is there any controller left shown?
